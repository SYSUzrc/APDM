...
class Ihaze:
    ...
    def get_loaders(self, parse_patches=True,):
        ...
        print("===> evaluating outdoor nhaze test set...")
        filename = 'nhaze.txt'

        train_dataset = InhazeDataset(
                                          n=self.config.training.patch_n,
                                          patch_size=self.config.data.image_size,
                                          transforms=self.transforms,
                                          parse_patches=parse_patches)
        val_dataset = InhazeDataset(    n=self.config.training.patch_n,
                                        patch_size=self.config.data.image_size,
                                        transforms=self.transforms,
                                        parse_patches=parse_patches)
...
class InhazeDataset(torch.utils.data.Dataset):
    def __init__(self, patch_size, n, transforms, parse_patches=True):
        super().__init__()

        train_list = "ihaze/ihaze.txt"
